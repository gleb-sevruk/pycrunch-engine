...
class TestRunner:
    ...
    def run(self, tests):
        ...
        for test_to_run in tests:
            ...
            try:
                ...
                cov = CoverageAbstraction(
                    should_disable_coverage, self.coverage_exclusions, self.timeline
                )
                cov.start()
                ...
                coverage_for_run = CoverageRun(
                    metadata.fqn, -1, test_to_run, execution_result=result
                )
    ...
    def serialize_test_run(
        self, cov, fqn, time_elapsed, test_metadata, execution_result
    ):
        """
