...
class ConnectionWatchdog:
    ...
    async def watch_client_connection_loop(self):
        ...
        while True:
            logger.debug(
                'watch_client_connection_loop-> deciding if we need to close engine'
            )
            ...
            if time_interval > significant_sleep_time:
                logger.info(
                    'watch_client_connection_loop > significant_sleep_time elapsed, will skip next loop'
                )
            ...
            if self.initial_connection_established and self.connected_clients <= 0:
                logger.warning(
                    '!! No connection established within reconnection deadline'
                )
                logger.warning(
                    'Engine will exit. Please restart it again if you need to.'
                )
                sys.exit(1)
